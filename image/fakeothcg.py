...
for _dir in ['OTHCG','BGCG']:
    ...
    with open(rf'..\patch\res\{_dir}.DAT','wb') as ff:
        ff.write(memnewothcgbs[:0x3800])  
    pf=open(f'../patch/{_dir}.cpp','w',encoding='utf-8-sig')
    print('''
    ...
    def getdatas(bs):
        ...
        for i in range(count):
            ...
            fnames=[
                rf'C:\InnocentGrey\カルタグラ FHD\files\image\{_dir}_NEW\\'+names[i],
                rf'C:\InnocentGrey\カルタグラ FHD\files\image\{_dir}_NEW\\'+names[i]+'.cz',
            ]
            for _fn in fnames:
                if os.path.exists(_fn):
                    with open(_fn,'rb') as ff:
    ...
    with open(rf'C:\InnocentGrey\カルタグラ FHD\files\image\{_dir}.PAK','rb') as ff:
        othcgbs=ff.read()  
    oldfileoffset,oldfilesize,_,_=getdatas(othcgbs)        
    newfileoffset,newfilesize,newcz,newczdata=getdatas(memnewothcgbs)  
    with open(rf'..\patch\res\{_dir}.IMG','wb')  as ff:
        ff.write(newczdata)
    pakoffsetnew2old={}
    for i in range(len(newfileoffset)):
        pakoffsetnew2old[newfileoffset[i]]=oldfileoffset[i]
    print(f'std::unordered_map<int,std::pair<int,int>>newcz{_dir}={{',file=pf)
    for k in newcz:
        print('{',k,',{'+newcz[k][0]+','+newcz[k][1]+'} },',file=pf)
    print('};',file=pf)

    print(f'std::unordered_map<int,int>pakoffsetnew2old{_dir}={{',file=pf)
    for k in pakoffsetnew2old:
        print('{',k,',',pakoffsetnew2old[k],'},',file=pf)
    print('};',file=pf)

    print(f'std::unordered_map<int,int>oldfilesize{_dir}={{',file=pf)
    for k in oldfilesize:
        print('{',k,',',oldfilesize[k],'},',file=pf)
    print('};',file=pf)
    print(f'std::unordered_map<int,int>newfilesize{_dir}={{',file=pf)
